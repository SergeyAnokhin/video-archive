...
- **Background jobs** — a two-lane worker (one CPU-bound lane, one network-bound lane for AI tagging) runs `rescan`, `convert`, `preview`, `tag`, `backup`, `restore`, `cleanup`, `optimize_db`.
  ...
  - A `convert`/tuning job's log also shows the exact ffmpeg command line (copy/paste into a terminal to reproduce the encode yourself), the source directory/file at the top, and each file's original vs. resulting size (plus a job-wide total for a directory-scope run).
