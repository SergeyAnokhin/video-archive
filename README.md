...
- **Background jobs** — a single sequential worker runs `rescan`, `convert`, `preview`, `tag`, `backup`, `restore`, `cleanup`, `optimize_db` with live per-item progress, cooperative cancellation, an SSE-streamed event log (Jobs modal + Log Viewer), and 24-hour retention for finished jobs.
