...
- **Library UI** — directory tree, card grid, search, and a per-file info panel.
  ...
  - A directory's always-visible "..." menu holds rescan, rescan + media info, "generate conversion script" (see Conversion below), and "delete orphaned previews" — a dry-run-then-confirm cleanup (current folder only, non-recursive) for `.jpg` collages left behind by a renamed/deleted/moved video.
