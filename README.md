...
- **Backend health & load indicator** — a pulsing status dot next to the app title (green = responding, amber = slow to answer within a configurable timeout — busy, not down — red = unreachable after a longer retry; both timeouts in Settings → Network) plus a compact CPU/memory/SMB-network gauge fed by `GET /api/system/stats` (whole process tree including ffmpeg children; cumulative SMB bytes counted across every read *and* write path). Click cycles four display modes (numbers+bars / rings / vertical bars / glowing icons), persisted across restarts; exact values always in hover tooltips.
