...
**Roadmap Stages 1-8 ("Skeleton", "Local Source, Scan, Browsing", "Job Infrastructure", "Conversion", "Preview Generation", "Tagging and Search", "SMB Source and Playback", "Backup, Restore, Maintenance, Source Switching") are implemented.** `frontend/` and `backend/` run together from the repository root: a Vite + React + TypeScript shell with a dark Strict theme, a responsive top bar, and EN/RU i18n, talking to a FastAPI backend with a schema-versioned SQLite database. On top of that:
...
See [docs/roadmap.md](docs/roadmap.md) for what comes next (Stage 9: theming polish, mobile refinement, optional similar-video detection/WebDAV/batch tagging).
