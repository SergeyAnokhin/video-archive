...
- **Sources** — several saved sources (`local` folders, `smb` shares, or `webdav` shares) at once, switchable from Settings → Source (configure/connect) and Settings → Saved sources (list, switch, forget) without losing each one's own data.
  - Settings → Source shows the active source's live connection status (reachable or not, and for `smb`, whether "direct access" is actually in effect right now) and doubles as its edit form: changing connection parameters (host/port/credentials/direct access/verify TLS certificate/name) while the source itself stays the same reconnects in place — no backup, no rescan.
  ...
  - All file access (scan, convert, preview, tag, playback, backup) goes through the uniform [`backend/app/sources/`](backend/app/sources/) layer, so every feature behaves identically across all three protocols.
  ...
  - Conversion's own write side has a matching, separately opt-in global switch (Settings → Conversion, "direct network write"): when on, and the source above is also direct-access-reachable, a converted file is written straight to the share and committed with a raw rename instead of a download-then-reupload round trip — off by default, always falls back automatically.
  - A `webdav` source (e.g. Synology DSM's WebDAV Server — see [`docs/webdav-setup.md`](docs/webdav-setup.md) for setup) talks plain HTTP (`PROPFIND`/`GET`/`PUT`/`MKCOL`/`DELETE`/`MOVE` via `httpx`, no third-party WebDAV library); it has no direct-access fast path, and can optionally skip TLS certificate verification (Settings → Source) for a server using a self-signed HTTPS certificate.
- **Library UI** — directory tree, card grid, search, and a per-file info panel.
  ...
  - Per-file info panel: media info via on-demand ffprobe (retried once automatically, and flagged in the UI rather than left blank, if it still can't be read — e.g. right after a conversion) plus all per-file actions (generate preview, tag, convert, tune, similar, move, delete).
  - Every card in the grid also has its own delete button (with confirmation), not just the info panel.
  ...
  - Folders can be created, deleted (if empty), and starred as favorites — favorited folders and a recently-viewed-from-folder History both surface as one-click "move this video here" buttons (drilling into subfolders via a small popover when a favorite has any, with its own "new subfolder" button to create a destination on the spot) in the player/info panel.
  - The directory toolbar's secondary actions (convert/preview/tag folder, rescan, rescan + media info, "generate conversion script" — see Conversion below, "delete orphaned previews" dry-run-then-confirm cleanup for `.jpg` collages left behind by a renamed/deleted/moved video, plus History/New folder/Sort once space is tight) all live in one "⋮" menu; on narrower widths, buttons collapse into it in two steps (folder actions first, then History/New folder/Sort) instead of all disappearing at once, and the toolbar wraps to its own line if the breadcrumb doesn't leave room.
...
- **Background jobs** — a two-lane worker (one CPU-bound lane, one network-bound lane for AI tagging) runs `rescan` (plain, or `rescan_with_media_info` to also probe/cache ffprobe technical data for every video touched), `convert`, `preview`, `tag`, `backup`, `restore`, `cleanup`, `optimize_db`.
  ...
  - A `convert`/tuning job's log also shows the exact ffmpeg command line (copy/paste into a terminal to reproduce the encode yourself), the source directory/file at the top, and each file's original vs. resulting resolution and size — in human-readable KB/MB/GB, not raw bytes (plus a job-wide total size for a directory-scope run).
  - A `convert` job reports live progress within the file currently encoding (parsed from ffmpeg's own `-progress` output) as a second, finer bar alongside the usual file-N-of-M one; cancelling now kills an in-flight ffmpeg process right away instead of only being noticed once it finishes on its own.
  - A job card never shows a raw job/file id — just the file name or directory being worked on, with the full path in a tooltip and a click that jumps to that directory.
  ...
  - A job still `running` when the backend last stopped is marked failed on the next startup, so it shows up as restartable (same button as any other failed job, same original parameters) instead of staying stuck forever — flagged distinctly from an ordinary failure (a warning icon on that job's row in the Jobs modal, vs. the plain failed icon for e.g. an ffmpeg error) so it's clear a restart, not a real error, is why it needs restarting.
  - The top-bar Jobs icon merges both signals into one: it pulses with a slow, calm glow while a job is running/queued, and rings red with a count badge whenever any job has failed (of any kind) — visible without opening the modal, and both can show at once.
- **Conversion** — saved ffmpeg profiles (codec, container, max dimension, CRF, drop-audio, `preset` speed/quality trade-off — ignored on hardware encoders, optional Intel Quick Sync/QSV hardware encoding on Windows with automatic software fallback) with a safe-replace pipeline.
  - A directory's "..." menu can also **generate a standalone PowerShell script** for a chosen profile (with its own overrides) instead of running a real job: the script reproduces the same ffmpeg command and safe-replace guarantee (temp file, exit-code/size check, atomic rename) so it can be copied and run on any machine with filesystem access to the same directory (e.g. over SMB), with no backend involved at all.
...
- **Previews** — two distinct kinds, configured on separate Settings tabs.
  ...
  - The **animated preview**: a GIF (per file, plus `folder-preview.gif` recursively per folder) shown on grid hover thumbnails. Its source mode is configurable — still frames or short video-clip bursts per segment — with adjustable segment duration and an optional crossfade transition. A folder's GIF draws from both videos and standalone images under it (not a video's own cover-art companion, which is excluded from the library at scan time) — a folder with only images still gets a preview, cycling through those images as static segments.
  ...
  - Frame extraction (collage, animated preview, and AI-tagging's own frame sampling) automatically uses Intel Quick Sync/VAAPI hardware *decode* when available ([`backend/app/hardware_decode.py`](backend/app/hardware_decode.py)), silently falling back to software per call if it fails — unlike encoding, decode output is bit-identical to software, so there's no quality tradeoff and no per-profile setting.
  - Collage/animated-preview frame extraction defaults to a fast nearest-keyframe seek (large speedup on CPU-only/slow hardware, at the cost of each sampled frame landing near rather than exactly at its timestamp); switchable back to exact-timestamp seeking (Settings → Preview → Animated preview tab, `frame_seek_mode`) since it also feeds the similar-files signature below.
...
- **Backend health & load indicator** — a pulsing status dot next to the app title (green = responding, amber = slow to answer within a configurable timeout — busy, not down — red = unreachable after a longer retry; both timeouts in Settings → Network) plus a compact CPU/memory/SMB-network gauge fed by `GET /api/system/stats` (whole process tree including ffmpeg children; cumulative SMB bytes counted across every read *and* write path — WebDAV traffic isn't counted in this particular gauge). Click cycles four display modes (numbers+bars / rings / vertical bars / glowing icons), persisted across restarts; exact values always in hover tooltips.
  - Settings → Performance also opens on a **CPU/memory/network history chart** — the same three signals as the live gauge, but plotted over a selectable 30 min/4h/12h/24h range with an in-chart color legend, sampled at whatever interval Resource monitoring (below it in the same tab) is already configured for. Network has no natural 100% ceiling, so it's shown relative to the highest throughput seen in the last 24h rather than a fixed scale. A range with more raw samples than fit on screen is downsampled by taking the max of each signal per time bucket, not an average, so brief spikes stay visible.
