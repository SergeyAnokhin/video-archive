...
- **Background jobs** — a two-lane worker (one CPU-bound lane, one network-bound lane for AI tagging) runs `rescan`, `convert`, `preview`, `tag`, `backup`, `restore`, `cleanup`, `optimize_db`.
  ...
  - A job still `running` when the backend last stopped is marked failed on the next startup, so it shows up as restartable (same button as any other failed job, same original parameters) instead of staying stuck forever.
