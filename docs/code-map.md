...
This repository currently contains the first real source setup, browsing flow, conversion workflow, and preview-generation workflow for Video Archive: a root npm orchestrator, a Vite-based React UI with source and preview settings, and a stdlib Python backend with local config loading, SQLite schema initialization, source configuration storage, saved conversion profiles, preview presets/settings, an async job queue, real conversion workers, real preview workers, persistent job items/events, and persisted tree/file metadata. Use this map to find the startup flow and the smallest files to extend first.
...
| [`frontend/src/App.jsx`](../frontend/src/App.jsx) | Main frontend flow for source setup, preview settings, live preview rendering, directory/file actions, conversion profile/mode launch modal, tree browsing, file listing, preview display, and the jobs modal with detail/items/live log updates. |
| [`frontend/src/api.js`](../frontend/src/api.js) | Frontend API helper for the live backend calls used by source setup, settings/preset loading, tree/file loading, preview loading, conversion profile loading, job creation, job detail loading, and job control actions. |
...
| [`backend/app/main.py`](../backend/app/main.py) | Stdlib HTTP entrypoint and request routing for source, settings, preview presets, tree, file preview, job, and log/event endpoints, including the SSE stream. |
| [`backend/app/config.py`](../backend/app/config.py) | Local config loading, including optional `backend/.env.local` overrides. |
| [`backend/app/db.py`](../backend/app/db.py) | SQLite connection helpers and idempotent schema initialization/migrations, including preview assets, preview settings, and preset persistence. |
| [`backend/app/job_service.py`](../backend/app/job_service.py) | Persistent job queue execution: queued/running terminal transitions, background worker, conversion profile snapshots, real conversion item handling, real preview item handling, job items, events, cancellation, and restart. |
...
| [`backend/app/library_service.py`](../backend/app/library_service.py) | Source-root scanning, persisted directory/file metadata upserts, preview invalidation on file changes, derived directory indicators, and file listing used by both browsing and queued scan jobs. |
...
| [`backend/tests/`](../backend/tests/) | Backend tests for config loading, schema init, default conversion profiles, preview settings/preset behavior, source persistence, password retention, scan/rescan, async job execution, preview/conversion state updates, cancel/restart, tree derivation, and file metadata resets. |
