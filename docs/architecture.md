...
| `smb_backend.py`'s `_smb_lock` serializes every SMB call in the process; a call stuck on a hung socket wedges all others until restart. `get_lock_status()`/`force_release_lock()` back the Settings → Source "release lock" button, which swaps in a fresh lock (the stuck thread is abandoned, not freed) | `backend/app/sources/smb_backend.py`, `frontend/src/components/SourceSection.tsx` |
