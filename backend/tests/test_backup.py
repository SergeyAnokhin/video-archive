...
`backup`/`restore`/`cleanup`/`optimize_db`/`migrate_legacy_previews` job
handlers, against both `local` and `smb` sources (`fake_smb`).
"""
...
from app.jobs.migrate_legacy_previews import _legacy_preview_gif_relative_path, run_migrate_legacy_previews_job
...
def test_migrate_legacy_previews_job_renames_flat_gif_to_hierarchical(engine, source):
    _dir_id, file_id = _seed_library(engine, source["id"])

    legacy_gif = source["root"] / _legacy_preview_gif_relative_path("clips/clip_0000.mp4")
    legacy_gif.parent.mkdir(parents=True, exist_ok=True)
    legacy_gif.write_bytes(b"legacy gif bytes")

    job = service.create_job(engine, "migrate_legacy_previews", "maintenance", None, {})
    status, message = run_migrate_legacy_previews_job(engine, job)

    assert status == "completed"
    assert "Migrated 1 of 1" in message
    assert not legacy_gif.exists()
    new_gif = source["root"] / preview_gif_relative_path("clips/clip_0000.mp4")
...
def test_migrate_legacy_previews_job_skips_when_nothing_to_migrate(engine, source):
    _seed_library(engine, source["id"])
    ...
    assert status == "completed"
    assert "Migrated 0 of 1" in message
    assert "1 skipped" in message


def test_migrate_legacy_previews_job_never_overwrites_existing_new_gif(engine, source):
    _seed_library(engine, source["id"])

    legacy_gif = source["root"] / _legacy_preview_gif_relative_path("clips/clip_0000.mp4")
    legacy_gif.parent.mkdir(parents=True, exist_ok=True)
    legacy_gif.write_bytes(b"stale legacy bytes")

    new_gif = source["root"] / preview_gif_relative_path("clips/clip_0000.mp4")
    new_gif.parent.mkdir(parents=True, exist_ok=True)
    new_gif.write_bytes(b"already regenerated bytes")
    ...
    assert status == "completed"
    assert "Migrated 0 of 1" in message
    assert legacy_gif.exists()
    assert new_gif.read_bytes() == b"already regenerated bytes"
