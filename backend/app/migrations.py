...
Each migration is applied at most once, in key order; `app/db.py` tracks the
applied version in `schema_meta` and runs the pending ones from `init_db()`.

Convention: append a new numbered entry, never edit an already-applied one.
Use `IF NOT EXISTS` DDL where possible -- a retried run may re-execute a
statement.
"""
